...
/** 檔名含 draft（不分大小寫、獨立字）的 .md 不公開，例如 `2025-08-golden-dream draft.md` */
const isDraftFilename = (filename) => /\bdraft\b/i.test(filename);

const isPublishedMarkdown = (filename) =>
  filename.endsWith('.md') && !isCopyFilename(filename) && !isDraftFilename(filename);
...
const resolveMarkdownRelativePaths = (markdown, assetBase) => {
  ...
  const linkRe = /\[([^\]]+)\]\((\s*<?)([^)\s>]+)(>?)([^)]*)\)/g;
  const out2 = out1.replace(linkRe, (_m, text, pre, url, post, rest) => {
    const resolved = isRelativeUrl(url) ? joinUrl(base, url) : url;
    return `[${text}](${pre}${resolved}${post}${rest})`;
  });

  const htmlSrcRe = /(<(?:video|audio|source)\b[^>]*\bsrc\s*=\s*["'])([^"']+)(["'])/gi;
  return out2.replace(htmlSrcRe, (_m, pre, url, post) => {
    const normalized = String(url).replace(/^\.[\\/]/, '');
    const resolved = isRelativeUrl(normalized) ? joinUrl(base, normalized) : url;
...
async function listMarkdownFiles(dir) {
  ...
  for (const e of entries) {
    ...
    if (e.isFile() && isPublishedMarkdown(e.name)) files.push(full);
  }
  return files;
}
