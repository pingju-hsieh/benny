...
export default function PromoteTocNav({ entries, scrollRoot }) {
  ...
  return (
    <div className="w-full shrink-0 lg:w-[min(100%,240px)] lg:max-w-[240px]">
      {/* 手機：目錄列固定，不跟著整頁捲 */}
      <nav
        aria-label="漫步推薦分類"
        className="mb-4 border-b border-amber-100/90 pb-3 lg:hidden"
      >
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {entries.map((e) => (
            <button key={e.id} type="button" onClick={() => scrollTo(e.id)} className={linkClass(e.id)}>
              {e.label}
            </button>
          ))}
        </div>
      ...
      <nav
        aria-label="漫步推薦分類"
        className="hidden w-full rounded-2xl border border-amber-100/90 bg-white/90 p-3 shadow-sm ring-1 ring-amber-50/80 lg:block"
      >
